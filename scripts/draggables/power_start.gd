...
@onready var power_start_collision: Area2D = $PowerStart_Collision
...
func _physics_process(_delta: float) -> void:
	if not power_start_collision.get_overlapping_areas() and not draggable:
...
func _input(event) -> void:
	if event is InputEventMouseButton:
		if mouse_in and event.is_pressed() and inside and !player.carrying:
			...
			draggable = false
			drop.emit()
			if power_start_collision.get_overlapping_areas():
				last_pos = position
